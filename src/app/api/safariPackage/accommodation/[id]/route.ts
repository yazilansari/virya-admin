...
export const DELETE = async (req: NextRequest, { params }: { params: { id: string } }) => {
  ...
  try {
    connection.query("DELETE FROM room_types WHERE accommodation_id = ?", [id]);
    connection.query("DELETE FROM meals WHERE accommodation_id = ?", [id]);

    const [accomodationResult] = await connection.promise().query<ResultSetHeader>(
      "DELETE FROM accommodations WHERE id = ?",
      [id]
    );

    // Check if a row was deleted
    if (accomodationResult.affectedRows > 0) {
